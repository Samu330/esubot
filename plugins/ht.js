let handler = async (m, { conn, text }) => {
  ...
  if (m.chat in conn.hartatahta) throw 'Alguien todavía está esperando\nRespuesta\nEn este chat... Espere a que termine'
  else conn.hartatahta[m.chat] = true
  m.reply('_Estan esperando..._\n*Espere aproximadamente 1 minuto🙋🏻‍♂️*')
  try {
    let img = await ht(text ? text : ':v')
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '*© Samu330*\nMade with FFmpeg', m)
  } finally {
    delete conn.hartatahta[m.chat]
  }
}
handler.help = ['tahta <texto>']
handler.tags = ['tools']
...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    let layers = [
      ...
      textArgs('SAMU', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2-(text_h*${lh})`, w, h) + format + '[top]',
      textArgs('330', 'black', 'white', fsize, font, '(w-text_w)/2', `(h-text_h)/2`, w, h) + format + '[mid]',
